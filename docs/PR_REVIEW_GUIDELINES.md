...
- 只要 PR 新增或实质修改 `backend/src/main/java` 或 `frontend/src` 中的手写生产代码，就必须审查有效注释率；纯文档、测试、配置、迁移和生成代码 PR 标记为“不适用”，不得用这些文件冲抵生产代码注释率。
- PR 注释率按目标分支到当前 PR 头提交的新增行计算；实质修改在差异中表现为新增行，同样纳入统计。计算公式为“有效注释行数 ÷（有效注释行数 + 有效代码行数）”，结果不得低于 30%，不通过四舍五入把低于 30% 的结果判为合格。PR 同时修改前后端时必须分别统计并分别达到 30%，不得合并计算或互相抵消。
- 统计排除空行、Java 的 `package`/`import`、前端的 `import`/纯转发行、仅含括号或分隔符的行以及生成代码。后端的 Javadoc、块注释和行注释，以及前端的 TSDoc、JSDoc、块注释、行注释、JSX 注释和 CSS 注释，只有在说明业务原因、约束、状态、并发、安全、兼容、交互边界或失败后果时才计入。
- 许可证头、重复模板、逐行翻译语法、被注释掉的废弃代码和为凑比例添加的无意义注释不计入。不得通过修改无关文件或集中添加与本次改动无关的注释补足比例。
- 除 PR 差异达到 30% 外，新增或修改代码不得降低所属模块的有效注释率；合并后的后端手写生产代码整体须达到 `docs/backend-coding-standards.md` 规定的 30% 门槛，前端手写生产代码整体也须达到本节规定的 30% 门槛，并继续遵守 `docs/frontend-coding-standards.md` 的有效注释要求。
- PR 描述必须按后端、前端分别给出可复现的统计命令或工具、有效注释行数、有效代码行数和最终百分比；未涉及的一端标记为“不适用”。审查者需要抽查被计入的注释是否有效。未提供证据、统计口径错误、任一适用端实际低于 30% 或使用无意义注释凑比例时，必须提交 `Request changes`，不能仅作为非阻断建议。
...
8. 按“有效注释率门禁”分别核验前后端 PR 差异、所属模块和整体注释率均满足 30% 要求；缺少可复现证据或任一适用端不达标时必须 `Request changes`。
