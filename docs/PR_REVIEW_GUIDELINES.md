...
- 目标分支中未被本 PR 修改的历史代码不纳入本次比例，也不得仅因所属模块或仓库整体尚未达到 30% 而阻断当前 PR。历史注释债务由后续独立治理任务按模块补齐；当前 PR 只负责保证本次新增或实质修改的生产代码达到 30%，并同步修正其直接影响范围内已经失效、错误或与实现冲突的旧注释。
...
8. 按“有效注释率门禁”分别核验当前 PR 的前后端生产代码差异均满足 30% 要求；历史代码和仓库整体比例不作为本次阻断条件，缺少可复现证据或任一适用端不达标时必须 `Request changes`。
