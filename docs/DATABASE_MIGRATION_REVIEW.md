...
| AI 审查助手 | 默认只读分析 OpenSpec 与 SQL，输出问题和验证建议；审查阶段不得连接数据库、执行 SQL、修改迁移文件或批准迁移。只有 A 已完成静态审查并在当前会话明确授权时，才可按第 6 节的环境守卫使用 A 专属验证库执行受控验证；该授权不包含共享库迁移、发布或提交。 |
...
AI 默认仍处于只读审查角色。只有 A 在当前会话明确授权执行迁移验证后，AI 才可以代 A 使用 `.env.migration-check` 连接专属验证库，并且必须同时满足以下守卫：

- 目标数据库名必须精确为 `cinewise_migration_check`，账号必须精确为 `cinewise_migrator`；任一不符立即停止。
- 只允许执行 Flyway `info`、`migrate`、`validate`、重复 `migrate`，以及验证表结构、索引、CHECK、字符集、排序规则和正反约束用例所需的 SQL。
- 首次迁移前必须确认目标库为空或仅包含可安全重建的验证历史；发现业务表、来源不明数据或环境标识不符时立即停止，不得自行清理。
- 不得读取、回显、记录或提交密码、主机、连接串及其他密钥；命令输出和验证记录必须脱敏。
- 不得连接或迁移共享 `cinewise`、生产库或其他数据库，不得修改日常 `.env`，不得把 `FLYWAY_ENABLED=true` 持久化。
- 验证授权不等于共享库发布、Git 提交、推送或执行授权；这些操作仍须由 A 分别明确授权。
