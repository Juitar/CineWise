# travel-task-reminder Specification

## Purpose
TBD - created by archiving change travel-reminder-experience. Update Purpose after archive.
## Requirements
...
#### Scenario: 退款先到且影院 ID 非法
- **GIVEN** D 尚未为该 `orderId` 创建任务且退款失效事件的 `cinemaId` 非法
- **WHEN** D 处理该事件或 A 调用 `ensureTaskCancelled`
- **THEN** D 仍创建或保留 `CANCELLED` 墓碑，写入 `cinema_id=NULL`
- **AND** 后续迟到支付事件不得重开任务，路线查询返回明确不可用
...
- **WHEN** 历史任务或退款先到墓碑的 `cinema_id` 为 `NULL`
- **THEN** 任务可保留，但路线查询返回明确不可用
