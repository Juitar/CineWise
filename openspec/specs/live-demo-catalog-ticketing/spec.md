# live-demo-catalog-ticketing Specification

## Purpose
TBD - created by archiving change live-demo-catalog-ticketing. Update Purpose after archive.
## Requirements
...
#### Scenario: 已保留历史场次不阻塞后续清理

- **GIVEN** 最早结束的 100 个 `demo-seed` 场次均已有订单，且下一条场次无订单、所有座位均为未锁定的 AVAILABLE
- **WHEN** 清理任务以 100 为批量上限执行
- **THEN** 系统跳过有订单的场次并将下一条场次纳入候选、完成清理
- **AND** 删除前仍执行加锁复核，候选筛选不作为交易安全判断的唯一依据
