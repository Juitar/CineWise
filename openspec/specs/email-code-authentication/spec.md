# email-code-authentication Specification

## Purpose
TBD - created by archiving change email-code-authentication. Update Purpose after archive.
## Requirements
...
### Requirement: 登录页必须支持邮箱验证码登录

登录页 SHALL 在同一路由提供密码登录和邮箱验证码登录切换。验证码方式 SHALL 通过认证模块发送 `purpose=LOGIN` 的验证码，并提交 `clientRequestId/email/code` 到 `/api/v1/auth/login/email`；管理员仍只能使用密码入口。

#### Scenario: 普通用户使用邮箱验证码登录

- **WHEN** 用户切换到验证码登录，获取并提交有效的 6 位 `LOGIN` 验证码
- **THEN** 前端查询 `/auth/me` 更新全局身份，并按现有安全 `returnUrl` 规则跳转

#### Scenario: 登录验证码发送冷却

- **WHEN** 服务端返回剩余冷却秒数，或发送结果暂时未知
- **THEN** 发送按钮在冷却结束前保持禁用；结果未知时提示先检查邮箱，不自动重发

#### Scenario: 登录验证码邮箱格式不合法

- **WHEN** 登录邮箱为空或格式不合法
- **THEN** 页面禁用发送验证码按钮，不调用验证码发送接口

#### Scenario: 登录验证码请求统一成功

- **WHEN** 验证码发送接口返回统一成功数据
- **THEN** 页面只显示“请检查邮箱”，不显示“验证码已发送”，也不据此判断邮箱是否已经注册

#### Scenario: 登录验证码无效

- **WHEN** 服务端返回 `201002`，表示验证码错误、过期、达到尝试上限或已经使用
- **THEN** 页面显示统一中文提示，不建立会话，并允许用户重新获取验证码

#### Scenario: 验证码登录结果未知

- **WHEN** 验证码登录 POST 超时或网络中断
- **THEN** 前端只查询 `/auth/me`；查询仍失败时只提供重新查询入口，不自动重发登录请求
