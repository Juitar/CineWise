# responsive-login-experience Specification
...
#### Scenario: HTTP IP Demo 生成降级请求标识

- **WHEN** 页面运行在不提供 `crypto.randomUUID()` 的 HTTP IP 非安全上下文
- **THEN** 前端生成长度不超过 64 的唯一字符串作为 `clientRequestId`，继续执行 `/csrf`、`/login/password`、`/auth/me`，且不把该降级值用于任何安全随机用途
