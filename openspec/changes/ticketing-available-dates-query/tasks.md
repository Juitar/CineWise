...
- [ ] 2.3 在现有 `ShowController` 新增 REST 入口及独立响应 DTO；验证：字符串 ID、缺参/非法参数 400、`dates` 数组契约已通过认证上下文测试；匿名公开仍待 C 在唯一安全链登记该 GET 路径。
- [x] 2.4 同步 OpenAPI 和 A 提供给 C 的成功 Mock 夹具；验证：字段精确且不含内容、库存、认证或交易敏感数据。

## 3. 测试与交付

- [ ] 3.1 补充 H2 Application/HTTP 集成测试，覆盖聚合、排序、时间边界、停售、跨影片影院、空结果、缺参和非法 ID；除匿名公开权限外均已通过，匿名场景待 C 安全 matcher。
...
- 契约影响：新增 `GET /api/v1/shows/available-dates?movieId=&cinemaId=`；不改变既有 `/shows`、数据库、权限规则、Agent Tool 或 D 内容事实。接口设计要求匿名公开，但 C 的唯一安全链尚未登记新路径，当前匿名请求实际返回 401。
...
- 未验证事项、剩余风险和后续负责人：C 需在现有唯一 `applicationSecurityFilterChain` 中将该 GET 路径设为 `permitAll` 并复核匿名 200；显式 MySQL 环境测试本轮未执行；前端 AI 后续按 `data.dates[{date,showCount}]` 接入。A 不修改 C 安全链，也未连接数据库或执行 SQL。
