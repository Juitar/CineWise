...
- [x] 1.6 C 已确认 `sys_user`、`sys_login_log` 的字段、索引、可空性、状态、30 天登录日志有限保留期物理删除例外和兼容要求；A 已确认 `sys_login_log` 作为只追加日志不增加 `update_time`。
- [x] 1.7 A 已为 `sys_user`、`sys_login_log` 正式分配 `V006__create_auth_user_and_login_log_tables.sql`；`V005` 已被票务占用。C 编写本地 SQL 草案，A 审查通过前不提交、不推送、不执行。
...
- [x] 4.1 扩展公共 `apiRequest`，实现 CSRF Token 内存管理、并发获取合并、写请求 Header、登录/登出清理、`201009` 换新和并发 401 单次处理，不自动重放写请求。
- [x] 4.2 创建 `modules/auth` 的 `CurrentUser`、登录 DTO、API 和共享登录 Hook，登录结果未知时只查询 `/auth/me`。
- [x] 4.3 创建 `shared/auth` 当前用户 Provider、启动恢复、登出清理、角色判断和私有状态清理入口。
- [x] 4.4 实现安全 `returnUrl` 工具，拒绝协议、双斜线、反斜线、控制字符和越权管理路径。
...
- [x] 5.1 覆盖登录 DTO、CSRF Header、并发 CSRF 获取、并发 401 单次处理和 403 保留会话。
- [x] 5.2 覆盖登录提交防重复、结果未知查询 `/auth/me`、匿名后允许用户主动重试和密码清理。
