...
- [ ] 3.3 等待 D 新增 `RecommendationPlanResult` / `RecommendationPlan` 的兼容公开 Tool 入口，补齐影片名称、影院名称、评分、距离、预计路程的空值、脱敏和降级规则，并提供成功、空结果、降级三套夹具；B 切换并验证 `rankMoviePlan` 映射后，D 再删除 `FixedRecommendationResult` 旧入口。D 的 PR 不修改 B 类型。
