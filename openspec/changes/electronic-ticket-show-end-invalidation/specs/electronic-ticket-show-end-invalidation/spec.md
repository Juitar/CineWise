...
系统 SHALL 在 `movie_show.end_time <= 当前业务时间` 时，将关联已支付订单的 `VALID` 电子票迁移为 `INVALIDATED`，并写入 `invalidated_time`。业务时间 SHALL 由注入的 `Clock` 取得。

#### Scenario: 结束边界

- **GIVEN** 已支付订单关联一张 `VALID` 电子票，场次 `end_time` 等于当前业务时间
- **WHEN** 电子票结束失效任务执行
- **THEN** 电子票状态为 `INVALIDATED`，其 `invalidated_time` 等于当前业务时间且版本仅增加一次
...
电子票页面 SHALL 对 `INVALIDATED` 票显示“影片已结束，电子票已失效”，且不得渲染可用二维码。

#### Scenario: 用户查看自动失效电子票

- **GIVEN** 电子票查询结果状态为 `INVALIDATED`
- **WHEN** 用户打开电子票页面
