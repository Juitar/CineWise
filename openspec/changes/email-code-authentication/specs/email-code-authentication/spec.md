...
系统 SHALL 生成 6 位数字验证码，只保存使用独立必填密钥计算的 HMAC-SHA-256 摘要。验证码 SHALL 在 5 分钟后过期，最多允许 5 次失败校验；成功消费或达到上限后立即失效。同一验证码并发消费 SHALL 只有一个请求成功。
...
系统 SHALL 使用独立必填的 `AUTH_INVITE_HASH_SECRET` 对邀请码计算 HMAC-SHA-256 摘要，并只按摘要查询。数据库、响应、普通日志、OpenSpec 和代码 SHALL NOT 保存或输出邀请码明文。
