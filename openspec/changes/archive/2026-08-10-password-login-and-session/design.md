...
HTTPS 安全上下文优先使用 `crypto.randomUUID()` 生成登录 `clientRequestId`。域名备案完成前的 HTTP IP Demo 中，浏览器可能不提供该方法；前端使用时间、页面内递增序号和普通随机片段生成长度不超过 64 的降级值。该值只用于请求追踪和幂等，不作为密码、Token、验证码、签名或其他安全随机值；恢复登录结果时仍沿用原值且不自动重发密码。
...
- [HTTP IP Demo 与 Secure Cookie、Web Crypto 能力冲突] → 长期入口仍迁移到 HTTPS；备案完成前通过独立环境配置关闭 Secure Cookie，并为缺失 `crypto.randomUUID()` 的登录页面提供非安全用途的请求 ID 降级，公网 HTTP IP 验证完整登录请求顺序。
