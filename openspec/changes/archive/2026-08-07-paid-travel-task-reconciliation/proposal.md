...
- 新增有界配置、调度入口、批处理报告和正常、重复、分页、失败隔离测试。
...
- D 已交付 `ensureTaskCancelled(OrderInvalidated)`；A 在 `refunded-travel-task-reconciliation` 完成退款取消补偿和竞态联调后，默认启用本调度入口。
...
- 非 PAID、窗口外、上下文缺失和扫描前已退款订单不会被 A 伪造成有效支付事件；退款竞态由 D 取消墓碑和 A 的 REFUNDED 对账共同收敛。
