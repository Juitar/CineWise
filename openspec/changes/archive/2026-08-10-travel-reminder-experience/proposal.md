...
支付后的电子票目前没有可恢复的出行提醒任务，天气、通用交通建议和基础路线也没有统一的真实数据接入与失败处理方式。PRD 要求第 5 天交付轻量出行和工具失败兜底；本变更把 D 负责的任务、建议、提醒和查询能力定为可实现、可联调的范围。
...
- D 实现用户主动发起的单条高德真实路线 Provider。仅在用户完成定位授权或主动提交手动起点，并确认第三方位置共享说明后调用；路线仅使用本次的一次性位置或手动地点，不保存精确位置、路线折线或途经点。未配置、超时或失败时只返回明确标记的 Demo 降级结果或路线不可用，不冒充真实路线。
...
- `travel-advice-query`: 天气、通用交通建议和基础路线的只读查询、来源时效、隐私和降级规则。
...
- 外部依赖：天气使用影院区域、城市和行政区码的本地映射查询；高德路线服务使用受控配置、一次性起点和影院终点。路线的精确起点、折线和途经点不写 MySQL、Redis、日志、画像、快照、URL 或 Agent 轨迹。
