...
- 响应包裹成功条件为 `code=0` 且存在 `data`；未使用不存在的 `success=true` 字段判断成功。
...
| 影片时长 | `data.movies[].dur` | 正整数分钟；只用于 A 计算本地预计结束时间，不是 Provider 的真实散场时间。 |
| 参考标价 | `shows[].plist[].vipPrice` | 返回数字字符串；只作为 `listedPrice`，不能写为本地交易价格。 |
| 展示资料 | `lang`、`th` | 可选的语言和影厅文本；`th` 可作为本地沙箱影厅的展示/命名参考，但不是稳定外部影厅 ID。 |
