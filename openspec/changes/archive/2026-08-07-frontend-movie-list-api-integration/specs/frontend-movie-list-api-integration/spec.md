...
### Requirement: 查询切换必须显示骨架且旧响应不得覆盖新条件

系统 SHALL 处理首次加载、条件切换骨架、失败恢复、手动重试、取消和离线状态。筛选、搜索或分页变化后的请求完成前，页面 SHALL 显示卡片骨架并隐藏旧影片、旧来源、空状态和分页；旧请求结果 MUST NOT 覆盖当前条件对应的结果。

#### Scenario: 已有数据时切换查询条件

- **GIVEN** 页面已经显示一页影片
- **WHEN** 用户切换筛选、提交搜索或翻页且新请求尚未完成
- **THEN** 页面显示影片卡片骨架
- **AND** 不显示上一次查询的影片卡片、来源提示、空状态和分页
- **AND** 请求成功后只显示新条件对应的结果
