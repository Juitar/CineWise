...
#### Scenario: 已发布迁移只由应用代码使用
- **WHEN** A 已审查并发布 V012 `agent_action` 迁移
- **THEN** B 只通过自身 Repository 与 MyBatis 映射使用该表，不修改已发布 SQL 或访问其他 Owner 的表
- **AND** 本 change 仍在 GitHub Actions MySQL 8.4 `cinewise_agent_it` 验证自己的 CAS、并发和恢复行为
