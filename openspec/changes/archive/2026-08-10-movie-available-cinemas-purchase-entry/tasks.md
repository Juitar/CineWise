...
- [x] C 为首页和 `/movies` 卡片增加语义化入口，首页影院进入既有影院详情，并在 `/movies/:movieId` 实现影片详情/选影院页；验证：点击、键盘和移动端测试。
- [x] C 处理影片详情和可售影院的合法空页、加载、404、失败、重试、`303004`、`306003`、过期、非实时来源、离线和来源时间；验证：页面状态测试。
