...
- 新增 `agent_event` 与 B 自有的 `agent_event_stream_cursor` 前向迁移、事件持久化和按十进制 `Last-Event-ID` 的续传；事件先持久化，再对 SSE 连接可见。
- 固化同一会话的数据库行锁、已提交水位线、游标归属/过期/跨会话/未来值处理、运行轨迹查询索引、V008 UUID 格式、不可变事件载荷、30 天过期与清理规则，作为 A 审查 V009 SQL 的依据。
...
- 明确 `stream.reset` 后的续传规则：运行详情只重建单次运行，下一次 `Last-Event-ID` 使用重置事件的会话水位线，避免多运行会话重复重置。
