...
- [x] 4.5 修正 `stream.reset` 会话水位线续传规则；验证：B 夹具断言重置事件 `eventId` 与 `payload.watermark` 一致，C 在 PR #53 评论 `5186922377` 复验先 GET 重建后以该值作为下一次 `Last-Event-ID`，不会因运行级 `lastEventId` 较小而循环重置，并拒绝不一致的水位线。
