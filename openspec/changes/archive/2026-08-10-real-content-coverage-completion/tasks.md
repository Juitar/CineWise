...
- [x] 1.3 D 已完成新增影片字段、`content_identity_mapping`、`cinema.city_name/provider_city_id`、`data_sync_log.city_name/provider_city_id/failure_category/lease_owner/lease_until` 的完整列、索引、180 天清理和兼容设计；A 已完成静态复核，并发布 V014。验证：2026-08-06 在 `cinewise_migration_check`（MySQL 8.4.11）完成首次 migrate、validate、重复 migrate、结构、索引、CHECK、字符集、排序规则及正反约束验证；共享 `cinewise` 已从 V013 升级至 V014 并完成 validate、重复 migrate 和结构核验，证据见 `docs/database-migrations/V014_MIGRATION_VALIDATION_2026-08-06.md`、`docs/database-migrations/V014_SHARED_MIGRATION_2026-08-06.md`。V014 只新增 PENDING 与兼容 CHECK，V015 严格状态约束仍未完成。
...
- [x] 2.9a D 按 A 确认新增 `DemoPurchaseCatalog` 公开只读目录：指定城市全部 `expiresAt > now` 的真实影院、最多三部真实影片、单一 `NETSTART_MAOYAN` 来源、DTO 自身稳定 ID 排序去重、空目录与 `303004` 区分。验证：`JdbcLiveDemoPurchaseCatalogAdapterTest` 覆盖全量影院、三部影片上限、`expiresAt == now` 排除、Demo/过期空目录和存储不可读；`ContentPurchaseQueryPortTest` 覆盖 DTO 的空列表、去重、排序与冲突拒绝；`ContentQueryServiceTest` 覆盖非法 `cityCode=100001`。
...
- [x] 3.6a A 经 C 确认实现答辩固定双城市选择：用户端顶栏、首页和影院页只支持长沙/杭州，并以 `location=430100/330100` 刷新；管理端同步页改为相同固定下拉。验证：城市 URL、内容查询、加载/空态、同步请求城市名以及移动端选择测试通过；不保存原始地点文本，不修改票务排期。
