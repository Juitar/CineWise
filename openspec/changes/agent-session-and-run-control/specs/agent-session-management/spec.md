...
### Requirement: 已清空会话不得接受新的消息提交
既有 POST SSE 消息提交入口 MUST 只接受当前用户的 `ACTIVE` 会话。已清空、不属于当前用户或不存在的会话 MUST 使用现有 `404 / 206005` 资源不存在语义拒绝，且不得新增运行、消息或事件。数据库占用 `active_run_id` 的最终条件 MUST 同时要求会话仍为 `ACTIVE`，避免清空与提交并发时重新占用已清空会话。

#### Scenario: 清空后再次提交消息
- **WHEN** 当前用户清空会话后，使用原 `sessionId` 调用既有 POST SSE 消息提交入口
- **THEN** 系统返回 `404 / 206005`
- **AND** 系统不新增运行、消息或事件，也不更新会话的 `active_run_id`
