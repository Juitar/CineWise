...
- [x] 2.2 C 新增 URL 查询参数规范化和 `useMovieList`，处理首次加载、保留旧数据刷新、错误、手动重试、取消和旧响应丢弃；验证：正常、非法参数、快速切换、卸载取消和失败重试测试通过。
...
- [x] 3.3 C 实现 Skeleton、空数据、失败、traceId、手动重试、保留旧数据刷新和离线只读提示；验证：各页面状态组件测试通过，不回填本地演示影片。
...
- 前端完整检查：`pnpm check` 通过；14 个测试文件、67 个测试全部通过，格式、Lint、TypeScript 和生产构建均通过；构建产物共 49 个带哈希 JS/CSS 文件。
