...
- `agent_action` 以 V013 为候选版本，但 A 尚未正式分配；完整 OpenSpec 必须先出现在远端并通过严格校验，随后由 A 静态审查 SQL。本 change 不自行新增或执行迁移。
...
- A：已确认建单 Tool、DTO、调用身份、稳定键、结果查询和结果未知语义；待 A 在远端 OpenSpec 可见后正式分配 V013、审查 SQL，并执行共享数据库验证。
- C：确认卡和结果事件的前端展示、CSRF 请求行为与断线恢复消费；本 change 不实现前端。
- 数据库：计划新增 B 拥有的 `agent_action`，但在 A 完成字段静态审查和版本分配前不得写 SQL 或执行迁移。涉及该表、CAS、唯一约束、并发确认和恢复时，必须在 GitHub Actions 的一次性 MySQL 8.4 `cinewise_agent_it` 中验证，H2 不替代该验证。
