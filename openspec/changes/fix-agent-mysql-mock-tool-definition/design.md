...
`MultiToolSupervisor` 在 Spring 初始化时将所有 `AgentToolExecutor` 交给 `AgentToolExecutorRegistry`，注册表要求每个执行器返回非空 `ToolDefinition`。MySQL 集成测试以 Mockito mock 替换推荐执行器，但只固定了 `targetName()`；`definition()` 的默认 `null` 导致上下文初始化失败。`Mockito.reset(...)` 同样会清除初始固定行为。
