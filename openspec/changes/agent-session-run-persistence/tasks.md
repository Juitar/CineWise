...
- [x] 4.2 Owner B：已把 `AgentPersistenceMySqlIntegrationTest` 和 `backend-mysql-integration.yml` 改为 CI 一次性 MySQL 8.4 方案：保留票务库，在同一容器创建 `cinewise_agent_it` 并只授权 `cinewise_ci`；测试只允许 `CINEWISE_MYSQL_AGENT_PERSISTENCE_IT=true` 且数据源精确为 `cinewise_agent_it / cinewise_ci` 时执行。GitHub Actions `Backend MySQL Integration` 第 67 次运行（`30907152988`）已通过：首次 Agent 测试验证空库 Flyway 初始化、V008 历史记录、数据库唯一约束、条件更新竞争和工具调用不占事务，第二次测试完成重复初始化验证；未连接或请求云端 `cinewise_migration_check` 凭据。
