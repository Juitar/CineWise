...
各Owner先在对应change和领域详设中确认本人表结构；A核对全局ID、时间和命名规则后统一分配Flyway版本。为支持代码审查，A可在对应feature change建立后生成标准格式迁移草案，但`FLYWAY_ENABLED`必须保持关闭，相关Owner确认且A明确授权前不得在任何数据库执行。`system_config`、`system_operation_log`等尚未建立feature change的表不得提前生成迁移。
