...
- 新增每天一次的受控同步：NetStart 返回的数据先标准化，再更新真实快照，并清掉旧缓存或写入新的真实缓存；固定 `demo-content-v1` 不更新、不进入缓存。
- 页面查询保持缓存优先：真实缓存、真实快照、明确标注过期的真实快照、固定 `demo-content-v1`、无数据；页面请求不直接调用 NetStart。
...
- NetStart 的公开网址、访问日期和学习用途声明可记录在 OpenSpec；Key、完整原始响应不写入 OpenSpec、仓库、日志、缓存或快照。每日配额、长期字段稳定性等未公开信息只记录为未知风险，不伪造确认结果。
