...
- [x] 3.3 D 补齐同步或调用记录的来源、资源、结果、耗时、数据时间、字段质量和降级层级；所有热映列表、详情和影院请求复用本地限流与一次短重试，写入前执行身份隔离，只持久化 accepted 内容并以固定原因记录 rejected 数量；同步日志统一按内容项统计 `total=success+failure`，满足 V004 的完成状态 CHECK；Redis 仅在 MySQL 事务提交后写入。验证：成功、超时、429、5xx、字段不合格和空 ID 冲突的记录均可审查且不泄露受限数据。
