...
对已通过启用门槛的查询，系统 SHALL 按“实时 Provider 成功 → 有效缓存 → 有效标准化快照 → 未超过最大陈旧时间的过期快照 → 唯一 `demo-content-v1` → `303004`”选择结果。Provider 超时、限流、网络失败、可重试服务端失败或字段校验失败 MUST 不改变该顺序；缓存、快照和 Demo 仍使用既有内容基线的 TTL、最大陈旧时间及来源封套规则。

#### Scenario: Provider 调用超时
- **GIVEN** 实时 Provider 在规定连接或读取时限内未返回
- **WHEN** 系统完成允许的一次短重试后仍失败
- **THEN** 系统依次尝试有效缓存、快照、允许陈旧快照和 `demo-content-v1`
