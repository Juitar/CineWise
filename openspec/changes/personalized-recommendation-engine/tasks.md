...
- [ ] 4.1 D 在 B、D 确认后升级 `RankMoviePlanTool` 的类型化输入输出，并保持只读、无 SSE、无模型调用和无 Agent 持久化访问；当前 D 已准备完整结果入口、错误处理和消费者夹具，但实际 Agent 白名单和适配器仍由 B 在 4.2 切换后，才能作为正式 `rankMoviePlan` 运行入口。验证：工具字段、错误码、`dataAt`、`expiresAt`、空方案和消费者夹具一致。
- [ ] 4.2 B 已完成普通完整推荐的工具注册、槽位转换和固定 `PLAN_CARD` 发送；实际 NEAREST 需要新的 run 暂停状态、CAS 恢复规则和 MySQL 迁移，本期不做，留待后续独立 change。验证：44 个 B 定向测试覆盖正常方案、空方案、放宽建议、过期数据、参数白名单和状态机传递。
