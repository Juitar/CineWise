...
`data_sync_log` 的状态和恢复规则固定如下：`PENDING` 表示请求已登记、尚未取得执行租约，三个计数均为 0，`error_code/error_summary/failure_category/finished_at/lease_until` 全为 NULL；获得租约后才转为 `RUNNING`，此时 `finished_at` 和三个错误字段均为 NULL，`lease_until` 必须非空。`SUCCESS/PARTIAL/FAILED` 都是终态，`lease_until` 必须清空，`finished_at` 必须非空且不早于 `started_at`。`SUCCESS` 的 `failure_count=0` 且三个错误字段均为 NULL；`PARTIAL/FAILED` 的 `failure_count>0`、`failure_category` 和 `error_code` 均非空，`error_summary` 只能是脱敏固定摘要或 NULL，绝不记录原始异常。

恢复任务每分钟扫描 `PENDING` 和租约过期的 `RUNNING`：多实例只能通过状态和版本条件更新取得同一条 PENDING 的租约，取得者才可调用一次 Provider；PENDING 不会产生重复调用。租约过期的 RUNNING 不得重新调用 Provider，而是条件更新为 `FAILED + INTERNAL`，设置 `total_count=1/success_count=0/failure_count=1`、固定 `303004` 错误码、脱敏摘要、`finished_at` 并清空租约。这样进程在 Provider 调用中退出时，原请求最终可查询，且同一请求标识永不触发第二次 Provider 调用；管理员只有使用新的 `clientRequestId` 才能发起新的同步。
...
| `data_sync_log` | `city_name VARCHAR(64) NULL`；`provider_city_id VARCHAR(32) NULL`；`failure_category VARCHAR(32) NULL`；`lease_until DATETIME(3) NULL`。 | 保留 `uk_sync_request(provider, request_id)`；新增 `idx_sync_city_resource_started(city_name, resource_type, started_at)` 与 `idx_sync_recovery(status, lease_until)`。状态 CHECK 为 `PENDING/RUNNING/SUCCESS/PARTIAL/FAILED`：PENDING 的三个计数均 0，错误字段、完成时间和租约均 NULL；RUNNING 的错误字段和完成时间均 NULL、租约非空；SUCCESS 的失败数及错误字段均为 0/NULL；PARTIAL/FAILED 的失败数大于 0、`error_code/failure_category` 非空；所有终态有完成时间且租约为 NULL。`failure_category` 仅 `NETWORK/RATE_LIMIT/PROVIDER_RESPONSE/DATA_VALIDATION/INTERNAL` 或 NULL。保留并替换 V004 原有计数、状态、完成时间 CHECK，使五种状态完整覆盖。 | 仅存城市名、内部 Provider 城市 ID、数值错误码和脱敏失败分类；不存地点原文、Provider URL、原始异常或原始响应。恢复任务只把租约过期 RUNNING 条件更新为 `FAILED + INTERNAL`，不重新调用 Provider；同一请求标识始终返回原任务。按既有 `create_time` 清理索引保留 180 天，清理只处理终态记录，PENDING/RUNNING 不得按时间自动删除。公开 API 绝不返回 `provider_city_id`。 |
