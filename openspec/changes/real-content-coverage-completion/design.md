...
受控启动验证可以把 `sync-movies-on-startup=false` 与固定 `sync-cities` 组合为单城市导入。影院搜索及补充详情最多会消耗完整的 10 req/min 本地预算，因此城市验证不得先执行影片启动同步；单条影院详情读取超时只允许一次短重试，随后保留搜索接口的合格名称和地址继续处理其余候选，不能因一条详情异常中断整批城市资料。
...
顶栏和管理端同步页只暴露长沙（`430100`）与杭州（`330100`）两个固定选项。前端将已选城市代码放入可分享的 `location` 查询参数，不保存地点原文；首页和影院页切换时清除分页并重新读取内容 API，其他业务页面切换时进入对应城市的影院页，避免把浏览城市错误作用于已创建订单、电子票或出行任务。

管理端仍向既有 `POST /api/v1/admin/content/sync` 提交城市名，后端继续由 D 的本地城市目录完成名称到内部 Provider 城市标识的受控解析。该 UI 改动不改变接口、认证、CSRF、幂等或结果未知恢复规则。杭州没有 A 的本地可售场次时，用户端只展示内容基础资料和“暂无可售场次”，不显示或伪造票务事实。
