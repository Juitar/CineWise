...
已确认：A 已接通 `cinemaArea` 摘要和支付事件登记方式，并正式分配 V007；C 已确认邮件端口及 Mock 查询语义；B、C 已确认只读工具和卡片边界。D 负责按本节起草 V007 前向迁移并提交 A 静态复核；A 负责决定是否授权验证和实际迁移。C 的邮件端口代码交付前，D 不接通邮件适配器或邮件联调。
...
1. D 按本设计第 6 节创建 `V007__create_travel_reminder_tables.sql`，只新增三张出行表，并提交 A 静态复核；不得修改 V001～V006，也不得自行执行 Flyway。
2. A 静态复核通过并明确授权后，在独立 `cinewise_migration_check` MySQL 8 库执行新迁移；已发布迁移不修改。
...
- A 已于 2026-08-04 正式分配 `V007`，专用于新增 `travel_task`、`travel_advice_snapshot`、`travel_notification_log` 三张表。版本分配不等于 SQL 执行、验证库迁移或共享库发布授权；不得修改 V001～V006，创建 V007 前向迁移后仍须提交 A 静态复核。
