...
- [x] 2.4 D 实现公开 Application DTO/Port，仅返回已映射候选及来源、时间、时效、`durationMinutes`、`auditoriumText` 和降级字段。验证：A 已确认 Port/DTO 字段和导入边界；`snapshots` 可含 `ACCEPTED` 或 `SANDBOX_REFERENCE`，前者可导入真实本地交易场次，后者仅供本地沙箱参考；`rejectedSnapshots` 最多返回 200 条并通过独立 `rejectedTruncated` 标记截断。
