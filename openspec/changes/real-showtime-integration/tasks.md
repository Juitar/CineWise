...
- [x] 2.4 D 实现公开 Application DTO/Port，仅返回已映射候选及来源、时间、时效和降级字段。验证：A 已确认 Port/DTO 字段和导入边界；`snapshots` 仅含可导入候选，`rejectedSnapshots` 最多返回 200 条并通过独立 `rejectedTruncated` 标记截断，显式返回质量和身份隔离结果，不写成功快照也不计入成功候选上限。
