...
- `qualityStatus`：`ACCEPTED` 或被隔离的失败原因摘要。
...
- `QueryResult` 同时返回 `rejectedSnapshots`、`rejectedTruncated`：拒绝明细最多返回 200 条，超出部分只通过 `rejectedTruncated=true` 表示；它不写入 D 的成功快照、不计入成功候选上限，也绝不能由 A 导入。每条隔离记录带 `qualityStatus`；身份隔离带稳定的 `303005/303006/303007`。
