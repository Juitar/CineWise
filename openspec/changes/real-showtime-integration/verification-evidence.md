...
- A 已确认：只导入 `ACCEPTED`、未过期、非降级、无回退且 `endTime > startTime` 的候选；本地价格由 A 配置，外部价格变化不得覆盖本地场次或订单；`QueryResult` 需要 `truncated`，候选按三元幂等键去重并限制 200 条；拒绝明细最多 200 条并通过独立 `rejectedTruncated` 标记。
