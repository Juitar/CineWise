...
- A 已确认：`ACCEPTED` 候选可导入本地交易场次；`SANDBOX_REFERENCE` 可按 `durationMinutes` 和 `auditoriumText` 创建明确标识的本地沙箱影厅、座位、预计结束时间和价格，但不得描述为外部真实交易事实；本地价格由 A 配置，外部价格变化不得覆盖本地场次或订单；`QueryResult` 需要 `truncated`，候选按三元幂等键去重并限制 200 条；拒绝明细最多 200 条并通过独立 `rejectedTruncated` 标记。
