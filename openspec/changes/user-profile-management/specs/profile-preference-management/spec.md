...
让登录用户管理自己的个性化开关和可解释偏好标签，并以可恢复的并发与幂等规则保存最小化行为摘要。
...
### Requirement: 账户删除必须停止画像使用并按保留期清理

系统 SHALL 仅通过 C 提供的类型化账户删除通知处理画像清理；登录退出不得触发该通知。收到通知后，D MUST 立即关闭个性化、删除摘要缓存，并使全部标签不再参与摘要或推荐；D 不得读取 `sys_user` Repository 或自行发起账户删除。偏好和软删除标签按 30 天清理，行为事件按 90 天清理；通知字段、重试和清理触发时机待 C 确认后实现。

#### Scenario: 账户删除后查询摘要
- **GIVEN** C 已成功通知 D 某用户账户删除
- **WHEN** B 或推荐模块在清理期内请求该用户摘要
- **THEN** 系统返回 `enabled=false`
- **AND** 不返回任何标签、行为或可识别资料

#### Scenario: 重复账户删除通知
- **GIVEN** D 已处理某用户的账户删除通知
- **WHEN** C 重放同一通知
- **THEN** 系统保持关闭和清理状态
- **AND** 不重复创建清理任务或恢复任何标签

### Requirement: 仅在有效同意状态下保存画像数据

系统 SHALL 在创建或更新开关、标签及行为摘要前取得 C 提供的个人数据保存同意状态。用户撤回同意后，系统 MUST 立即关闭个性化、删除摘要缓存并停止后续画像写入；D 不读取认证表或将“已登录”推断为已同意。

#### Scenario: 用户未同意保存个性化数据
- **GIVEN** C 返回当前用户没有有效同意状态
- **WHEN** 用户提交标签或受信任调用方提交行为事件
- **THEN** 系统拒绝保存且不创建画像缓存
- **AND** 返回码由 C 与 D 在确认公共错误码后补充

#### Scenario: 用户撤回同意
- **GIVEN** 用户已有启用的画像与缓存
- **WHEN** C 通知 D 同意已撤回
- **THEN** 系统立即关闭个性化并删除缓存
- **AND** 后续摘要只返回 `enabled=false`

### Requirement: 画像审计和日志必须最小化

系统 SHALL 对画像写入、摘要读取、账户删除、行为拒绝和清理结果记录最小审计信息。日志与指标不得记录标签完整值、原始 payload、会话正文、Cookie、JWT、邮箱、支付明细、精确位置或路线数据。
