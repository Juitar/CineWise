...
- [ ] 2.1 D 建立 `profile` 的 api/application/domain/infrastructure 分层、值对象、标签来源/状态/极性和错误映射；验证：架构测试和生产代码中文注释率达到团队要求。
- [ ] 2.2 D 实现默认偏好设置创建、本人开关与分页标签查询、新增、更新、单标签停用/恢复、软删除、归属校验和版本冲突返回；验证：覆盖首次并发访问、102001、202001、202002、202003、跨用户访问和字符串 ID 序列化。
- [ ] 2.3 D 实现 `If-Match`、`Idempotency-Key`、版本条件更新和重复响应恢复；验证：覆盖并发编辑、相同幂等键重放、不同幂等键的重复标签和缓存失效。
- [ ] 2.4 D 实现 `profile_write_request` 的请求摘要哈希、同一本地事务内的响应恢复、同键并发串行化和同键不同请求拒绝；验证：进程重启后重放、并发相同键、事务失败无残留、不同键和敏感字段不落库均通过。
- [ ] 2.5 A 在隔离 MySQL 8 环境验证 V010 前向迁移；D 补齐 Repository 集成验证。验证：四表的唯一键、CHECK、索引、软删除、限定清理、同事务幂等恢复和 UTC 时间语义均通过；不改已发布迁移。

## 3. 行为、摘要与缓存

- [ ] 3.1 D 实现受信任来源的最小行为事件接收、`eventId` 去重、24 小时归一化限制和固定权重累计；验证：重复事件、两个不同事件并发命中同一归一化键时只累计一次、事务失败后同一事件重试无残留、单次点击、阈值创建、`BEHAVIOR` 的 0.800/有效期约束、显式偏好优先、未受信任 `PAID_ORDER` 拒绝、无同意拒绝和失败不影响上游业务。
...
- [ ] 3.5 D 实现 C 已确认的账户删除通知处理和分期清理；验证：立即关闭、缓存失效、重复通知幂等、30 天画像数据清理和 90 天行为摘要清理均通过，且不访问 `sys_user` Repository。
