...
- [x] C：运行 Agent 定向测试和相关桌面/移动 Playwright；验证：Agent 定向单测 121/121、工作区 Playwright 10/10 通过。
...
- [x] C：复跑 Agent 定向测试、工作区 Playwright、`pnpm check`、后端 `verify`、OpenSpec 严格校验和 Git 检查；验证：基于最新 `origin/dev` 全部通过。
