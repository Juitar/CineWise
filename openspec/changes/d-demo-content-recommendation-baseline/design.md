...
### 2. Demo 数据复用 A 的共享固定数据

影片和影院数据库初始化由 A 的 `content-and-show-selection-flow` 固定种子负责，使用同一份共享固定数据生成 8 至 12 部影片和 3 至 5 家影院。影片和影院 ID、来源 ID、标题、城市、行政区和地址保持稳定；经纬度只为后续用户主动路线能力保留，不进入日志和推荐默认评分。

`demo-content-v1` 保留为共享数据文件的版本标识、Provider 回退数据或测试夹具；它不得再次向 `movie`、`cinema` 插入另一套记录。D 的 Provider、测试夹具和快照回退必须复用共享数据中的稳定业务 ID。加载时由统一 `Clock` 生成本轮 `dataTime` 和 `expiresAt`；自动测试使用固定 `Clock`，从而同时满足演示数据不过期和回归结果可重复。
...
- [共享固定数据与 A 场次数据 ID 不一致] → A 初始化共享影片、影院数据，D 复用稳定业务 ID；实现前记录 ID 对照和验证用例，确认前推荐只返回不可购内容候选。
...
4. A 的固定种子初始化共享影片、影院数据；D 复用该数据的稳定 ID 作为 Provider 回退和测试夹具。
...
- A 确认共享固定数据文件的维护位置和 ID 对照表；在此之前 D 只返回不可购内容候选。
