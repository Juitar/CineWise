...
public interface ContentSyncTaskPort {
    ...
    /**
     * 在资料事务内条件续租并锁住任务行，返回 false 时调用方必须回滚资料事务。
     *
     * <p>这不是普通查询：成功后该行锁一直保留到当前资料事务提交或回滚。恢复任务因此不能在旧 Worker
     * 写资料与提交之间插入 `FAILED`，避免任务终态和公开资料出现不一致。</p>
     */
    boolean lockAndRenewActiveLease(long syncId, String leaseOwner, LocalDateTime leaseUntil, LocalDateTime now);
