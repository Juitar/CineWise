...
public record RecommendationPlanResult(
    ...
    public RecommendationPlanResult {
        if (algorithmVersion == null || algorithmVersion.isBlank() || source == null || source.isBlank()) throw new IllegalArgumentException("版本和来源不能为空");
        ...
        dataAt = Objects.requireNonNull(dataAt, "dataAt 不能为空"); expiresAt = Objects.requireNonNull(expiresAt, "expiresAt 不能为空");
        if (!expiresAt.isAfter(dataAt) || plans.size() > 3) throw new IllegalArgumentException("推荐结果时效或数量不合法");
    }
}
