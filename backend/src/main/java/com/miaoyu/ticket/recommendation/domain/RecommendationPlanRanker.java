...
public final class RecommendationPlanRanker {
    ...
    public static List<RecommendationPlan> rank(
        ...
        List<RankedRecommendationCandidate> filtered = RecommendationCandidateFilter.filter(
                candidates, constraints, clock);
        if (filtered.isEmpty()) {
            return List.of();
        }
        BigDecimal min = filtered.stream().map(RankedRecommendationCandidate::price)
    ...
    public static RecommendationPlan nearest(List<RankedRecommendationCandidate> candidates,
        ...
        return RecommendationCandidateFilter.filter(candidates, constraints, clock).stream()
                ...
                .min(Comparator.comparingInt((RankedRecommendationCandidate candidate) ->
                                distanceMeters.get(candidate.cinemaId()))
                        .thenComparing(RankedRecommendationCandidate::price)
                        .thenComparing(RankedRecommendationCandidate::startTime)
                        .thenComparing(RankedRecommendationCandidate::showId))
                .map(candidate -> nearestPlan(candidate, distanceMeters))
                .orElse(null);
    }
    ...
    private static Scored scored(RankedRecommendationCandidate candidate, RecommendationConstraints constraints,
        ...
        double priceScore = min.compareTo(max) == 0 ? 100D
                : max.subtract(candidate.price()).multiply(BigDecimal.valueOf(100))
                        .divide(max.subtract(min), 6, RoundingMode.HALF_UP).doubleValue();
        double timeScore = timeScore(candidate, constraints);
        double ratingScore = candidate.rating() == null
                ? Double.NaN : candidate.rating().multiply(BigDecimal.TEN).doubleValue();
        double[] weights = weights(type);
        ...
        if (!Double.isNaN(ratingScore)) {
            numerator += ratingScore * weights[2];
    ...
    private static double[] weights(RecommendationPlan.PlanType type) {
        return switch (type) {
            case COMPREHENSIVE -> new double[] { .35, .35, .30 };
            case LOW_PRICE -> new double[] { .60, .25, .15 };
            case EARLY_TIME, NEAREST -> new double[] { .20, .65, .15 };
        };
    }
